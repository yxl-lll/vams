...
class FixQuery:
    @staticmethod
    def query(
        model: BaseModel, val: Any, instance: Table, total_query: Any, query: Any
    ):
        d_val = val.dict()
                        ...
                        match fields[1]:
                            case "like":
                                total_query = total_query.where(
                                    instance.c[field].like(f"%{fields[0]}%")
                                )
                                query = query.where(
                                    instance.c[field].like(f"%{fields[0]}%")
                                )
                            case "in":
                                ...
                                total_query = total_query.where(
                                    instance.c[field].in_(in_val)
                                )
                                query = query.where(instance.c[field].in_(in_val))
                            case "bt":
                                between_val = fields[0].split(",")
                                total_query = total_query.where(
                                    instance.c[field].between(
                                        between_val[0], between_val[1]
                                    )
                                )
                                query = query.where(
                                    instance.c[field].between(
