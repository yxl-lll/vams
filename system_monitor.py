    ...
    def check_system_health(self):
        ...
        if system_info:
            ...
            if system_info['cpu_percent'] > 80:
                health_status['warnings'].append(f"CPU使用率过高: {system_info['cpu_percent']}%")
                health_status['overall_status'] = 'warning'
            
            # 内存使用率检查
            if system_info['memory_percent'] > 85:
                health_status['warnings'].append(f"内存使用率过高: {system_info['memory_percent']}%")
                health_status['overall_status'] = 'warning'
            
            # 磁盘使用率检查
            if system_info['disk_percent'] > 90:
                health_status['errors'].append(f"磁盘使用率过高: {system_info['disk_percent']}%")
                health_status['overall_status'] = 'error'
        
        if db_info:
            # 数据库连接数检查
            if db_info['threads_connected'] > 100:
                health_status['warnings'].append(f"数据库连接数过多: {db_info['threads_connected']}")
                health_status['overall_status'] = 'warning'
            
            # 慢查询检查
            if db_info['slow_queries'] > 10:
                health_status['warnings'].append(f"慢查询数量较多: {db_info['slow_queries']}")
                health_status['overall_status'] = 'warning'
