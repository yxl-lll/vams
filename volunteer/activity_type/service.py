...
""" 分页查询 """
async def page_data(start: int = 1, limit: int = 10, query_activity_type: ActivityTypeQuery = None):
    try:
...
async def update(id: str, _activity_type: ActivityTypeUpdateBody):
    ...
    values['updated_at'] = datetime.now()
    try:
        await database.execute(activity_type.update().where(activity_type.c.id == id).values(values))
        return True
    except Exception:
        return False
...
async def one(id: str):
    return await database.fetch_one(activity_type.select().where(activity_type.c.id == id))
