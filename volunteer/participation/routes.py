...
from .service import page_data, add, delete, update, one, list_data, check_in, check_out, get_user_statistics, cancel_participation
from .model import ParticipationQuery, ParticipationBody, ParticipationUpdateBody, CheckInBody, CheckOutBody
...
async def _page(page: int, limit: int, activity_name: str | None = None, status: str | None = None):
    if activity_name is not None:
        activity_name = activity_name + "#like"
    total, list_data = await page_data(page, limit, ParticipationQuery(activity_name=activity_name, status=status))
    # 符合Layui表格要求的响应格式
    return {
        "code": 0,
        "msg": "",
...
async def _list(activity_name: str | None = None, status: str | None = None):
    if activity_name is not None:
        activity_name = activity_name + "#like"
    data = await list_data(ParticipationQuery(activity_name=activity_name, status=status))
    return {
        "code": 0,
        "msg": "",
        "count": len(data),
...
async def _add(
    _participation: ParticipationBody,
    current_user: Annotated[dict, Depends(auth_name)]
):
...
async def _check_in(id: str, check_in_data: CheckInBody):
    ...
    if result:
        ...
        return {
            "code": 1,
            "msg": "签到失败，记录不存在或状态不正确",
            "data": None
        }

@participation_route.post("/checkout/{id}")
async def _check_out(id: str, check_out_data: CheckOutBody):
    result = await check_out(id, check_out_data.check_out_time, check_out_data.service_hours)
    if result:
        return {
            "code": 0,
            "msg": "签退成功",
