from sqlalchemy import select, Table, Column, String, DATETIME, Integer, Float, func
from config import metadata, database
from datetime import datetime
from .model import ParticipationQuery, ParticipationBody, ParticipationUpdateBody
...
""" 分页查询 """
async def page_data(start: int = 1, limit: int = 10, query_participation: ParticipationQuery = None):
    try:
...
async def one(id: str):
    try:
        return await database.fetch_one(participation.select().where(participation.c.id == id))
    except Exception as e:
...
async def add(_participation: ParticipationBody, current_user: dict):
    try:
        logger.info(f"添加活动参与记录：活动ID={_participation.activity_id}，用户ID={current_user['user_id']}")
        ...
        if existing:
            logger.warning(f"用户{current_user['user_id']}已报名活动{_participation.activity_id}")
            return False
...
async def update(id: str, _participation: ParticipationUpdateBody):
    try:
        ...
        await database.execute(participation.update().where(participation.c.id == id).values(values))
...
async def check_in(id: str, check_in_time: str):
    try:
        ...
        record = await database.fetch_one(
            select(participation).where(
                (participation.c.id == id) &
                (participation.c.status == "registered")
            )
        )
        if not record:
...
async def get_user_statistics(user_id: str):
    try:
        ...
        total_activities = await database.fetch_val(
            select(func.count(participation.c.id))
            .where(participation.c.user_id == user_id)
        )
        
        total_hours = await database.fetch_val(
            select(func.coalesce(func.sum(participation.c.service_hours), 0))
            .where(participation.c.user_id == user_id)
        )
