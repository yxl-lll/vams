from sqlalchemy import select, Table, Column, Integer, String, DATETIME, DATE, Text, func, DECIMAL
...
""" 分页查询 """
async def page_data(start: int = 1, limit: int = 10, query_volunteer_profile: VolunteerProfileQuery = None):
    try:
        logger.info(f"查询志愿者档案，页码：{start}，每页：{limit}")
        
        # 基础查询
        base_query = select(volunteer_profile)
        total_query = select(func.count(volunteer_profile.c.id)).select_from(volunteer_profile)
...
async def delete(id: str):
    try:
        await database.execute(volunteer_profile.delete().where(volunteer_profile.c.id == id))
        return True
    except Exception:
        return False
...
async def update(id: str, _volunteer_profile: VolunteerProfileUpdateBody):
    ...
    values['updated_at'] = datetime.now()
    try:
        await database.execute(volunteer_profile.update().where(volunteer_profile.c.id == id).values(values))
        return True
    except Exception:
        return False
...
async def one(id: str):
    return await database.fetch_one(volunteer_profile.select().where(volunteer_profile.c.id == id))

""" 根据用户ID获取档案 """
async def get_by_user_id(user_id: str):
    return await database.fetch_one(volunteer_profile.select().where(volunteer_profile.c.user_id == user_id))

""" 更新服务统计 """
async def update_service_stats(user_id: str, service_hours: int, activity_count: int):
    try:
        await database.execute(volunteer_profile.update().where(volunteer_profile.c.user_id == user_id).values({
            'total_service_hours': volunteer_profile.c.total_service_hours + service_hours,
            'total_activities': volunteer_profile.c.total_activities + activity_count,
