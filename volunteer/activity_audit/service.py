from sqlalchemy import select, Table, Column, Integer, String, DATETIME, Text, func
from config import metadata, database
from datetime import datetime
from .model import ActivityAuditQuery, ActivityAuditBody, ActivityAuditUpdateBody
...
""" 分页查询 """
async def page_data(start: int = 1, limit: int = 10, query_activity_audit: ActivityAuditQuery = None):
    try:
        logger.info(f"查询活动审核，页码：{start}，每页：{limit}")
        base_query = select(activity_audit)
        total_query = select(func.count(activity_audit.c.id)).select_from(activity_audit)
        if query_activity_audit:
            total_query, base_query = FixQuery.query(
                query_activity_audit, val=query_activity_audit, instance=activity_audit,
    ...
    try:
        ...
        if query_activity_audit:
            _, base_query = FixQuery.query(
                query_activity_audit, val=query_activity_audit, instance=activity_audit,
                total_query=select(func.count(activity_audit.c.id)), query=base_query
            )
...
async def update(id: str, _activity_audit: ActivityAuditUpdateBody):
    ...
    if 'audit_opinion' in values:
        values['audit_remarks'] = values.pop('audit_opinion')
    try:
        await database.execute(activity_audit.update().where(activity_audit.c.id == id).values(values))
        return True
    except Exception:
        return False
...
async def one(id: str):
    return await database.fetch_one(activity_audit.select().where(activity_audit.c.id == id))

""" 根据活动ID获取审核记录 """
async def get_by_activity_id(activity_id: str):
    return await database.fetch_one(activity_audit.select().where(activity_audit.c.activity_id == activity_id))
...
async def get_audit_statistics():
    try:
        status_query = select(activity_audit.c.audit_status, func.count(activity_audit.c.id).label('count')).group_by(activity_audit.c.audit_status)
        status_stats = await database.fetch_all(status_query)
        today = datetime.now().date()
        today_query = select(func.count(activity_audit.c.id)).where(func.date(activity_audit.c.audit_time) == today)
