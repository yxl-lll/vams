...
from sqlalchemy import Table, Column, String, Text, Boolean, DateTime, func, select, update, delete, MetaData
...
notification_table = Table(
    ...
    Column("type", String(50), nullable=False, comment="消息类型: system, activity, reminder, announcement"),
    Column("priority", String(20), default="normal", comment="优先级: high, normal, low"),
...
async def page_data(database, page: int, limit: int, query_params: NotificationQuery) -> tuple[int, List[Dict[str, Any]]]:
    """分页查询通知"""
    offset = (page - 1) * limit
    ...
    if query_params.target_user_id:
        query = query.where(notification_table.c.target_user_id == query_params.target_user_id)
    ...
    query = query.order_by(notification_table.c.created_at.desc()).offset(offset).limit(limit)
...
async def update(database, id: str, notification: NotificationBody) -> bool:
    """更新通知"""
    try:
        query = notification_table.update().where(notification_table.c.id == id).values(
            title=notification.title,
            content=notification.content,
            type=notification.type,
            priority=notification.priority,
...
async def get_by_user_id(database, user_id: str, unread_only: bool = False) -> List[Dict[str, Any]]:
    """根据用户ID查询通知"""
    query = select(notification_table).where(notification_table.c.target_user_id == user_id)
    if unread_only:
...
async def mark_multiple_as_read(database, ids: List[str]) -> bool:
    """批量标记通知为已读"""
    try:
        query = notification_table.update().where(notification_table.c.id.in_(ids)).values(
            is_read=True,
            read_at=datetime.now(),
            updated_at=datetime.now()
        )
...
async def get_unread_count(database, user_id: str) -> int:
    """获取用户未读通知数量"""
    query = select(func.count()).select_from(notification_table).where(
...
async def get_notification_statistics(database) -> Dict[str, Any]:
    """获取通知统计数据"""
    try:
        type_query = select(notification_table.c.type, func.count(notification_table.c.id).label('count')).group_by(notification_table.c.type)
        type_stats = await database.fetch_all(type_query)
        
        priority_query = select(notification_table.c.priority, func.count(notification_table.c.id).label('count')).group_by(notification_table.c.priority)
        priority_stats = await database.fetch_all(priority_query)
        
        today = datetime.now().date()
        today_query = select(func.count()).select_from(notification_table).where(func.date(notification_table.c.created_at) == today)
...
async def send_system_notification(database, title: str, content: str, priority: str = 'normal') -> bool:
    """发送系统通知"""
    try:
        query = notification_table.insert().values(
            id=str(uuid.uuid4()),
...
async def send_activity_notification(database, title: str, content: str, target_user_id: str, priority: str = 'normal') -> bool:
    """发送活动通知"""
    try:
        query = notification_table.insert().values(
            id=str(uuid.uuid4()),
