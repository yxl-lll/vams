...
from .model import StatisticsQuery, VolunteerStatistics, ActivityStatistics, UserStatistics
...
async def get_activity_type_statistics(query: StatisticsQuery = None):
    ...
    try:
        ...
        type_stats_query = select(
            ...
            func.sum(volunteer_plans.c.max_participants).label('total_participants'),
            func.coalesce(func.sum(participation.c.service_hours), 0).label('total_service_hours')
        ).select_from(
            volunteer_plans.outerjoin(participation, volunteer_plans.c.id == participation.c.activity_id)
        ).group_by(volunteer_plans.c.activity_type)
        
        if query and query.start_date:
            type_stats_query = type_stats_query.where(volunteer_plans.c.activity_date >= query.start_date)
        if query and query.end_date:
            type_stats_query = type_stats_query.where(volunteer_plans.c.activity_date <= query.end_date)
        ...
        return [
            ActivityStatistics(
                ...
                total_participants=int(stat.total_participants) if stat.total_participants else 0,
...
async def get_user_statistics(query: StatisticsQuery = None):
    ...
    try:
        ...
        if query and query.user_id:
            user_stats_query = user_stats_query.where(participation.c.user_id == query.user_id)
        if query and query.start_date:
            user_stats_query = user_stats_query.where(participation.c.activity_date >= query.start_date)
        if query and query.end_date:
            user_stats_query = user_stats_query.where(participation.c.activity_date <= query.end_date)
