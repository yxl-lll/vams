...
async def _page(
    ...
    current_user: Annotated[dict, Depends(auth)] = None,
):
    ...
    total, list_data = await page_data(
        page, limit, query_params, current_user
    )  # <--- 传递 current_user
...
async def _list(
    current_user: Annotated[dict, Depends(auth)],
    activity_name: Optional[str] = None,
    activity_type: Optional[str] = None,
    user_id: Optional[str] = None,
):
...
async def _add(
    _volunteerPlans: VolunteerPlansBody, user_info: Annotated[any, Depends(auth_name)]
):
...
async def _update(
    id: str,
    _volunteerPlans: VolunteerPlansUpdateBody,
    user_info: Annotated[any, Depends(auth_name)],
):
