...
from .model import VolunteerPlansQuery, VolunteerPlansBody, VolunteerPlansUpdateBody
...
""" 分页查询 """
async def page_data(start: int = 1, limit: int = 10, query_volunteer_plans: VolunteerPlansQuery = None, current_user: dict = None):
    try:
        logger.info(f"查询活动计划，页码：{start}，每页：{limit}")
        base_query = select(volunteer_plans)
        total_query = select(func.count(volunteer_plans.c.id)).select_from(volunteer_plans)
        
        # --- 新增逻辑：根据当前用户角色过滤 ---
        if current_user and 'role' in current_user:
            base_query = base_query.where(volunteer_plans.c.creator_role == current_user['role'])
            total_query = total_query.where(volunteer_plans.c.creator_role == current_user['role'])
        # -------------------------------------

        if query_volunteer_plans:
            total_query, base_query = FixQuery.query(
                query_volunteer_plans, val=query_volunteer_plans, instance=volunteer_plans,
                total_query=total_query, query=base_query
            )
        list_data = await database.fetch_all(base_query.offset((start - 1) * limit).limit(limit))
...
""" 列表查询 """
async def list_data(query_volunteer_plans: VolunteerPlansQuery = None, current_user: dict = None):
    try:
        ...
        if query_volunteer_plans:
            _, base_query = FixQuery.query(
                query_volunteer_plans, val=query_volunteer_plans, instance=volunteer_plans,
                total_query=select(func.count(volunteer_plans.c.id)), query=base_query
            )
...
async def delete(id: str):
    try:
        await database.execute(volunteer_plans.delete().where(volunteer_plans.c.id == id))
        return True
    except Exception:
        return False
...
async def add(_volunteer_plans: VolunteerPlansBody, user_info: any):
    ...
    try:
        ...
        logger.info(f"活动计划 '{values['activity_name']}' 创建成功，ID: {values['id']}，状态: draft")

        # 2. 立即将活动状态更新为 'pending'（待审核）
        status_updated = await update_status(values['id'], 'pending')
        if status_updated:
            logger.info(f"活动 '{values['activity_name']}' 状态已从 draft 更新为 pending。")
        else:
...
async def update(id: str, _volunteer_plans: VolunteerPlansUpdateBody):
    ...
    try:
        await database.execute(volunteer_plans.update().where(volunteer_plans.c.id == id).values(values))
        return True
    except Exception:
        return False
...
async def one(id: str):
    return await database.fetch_one(volunteer_plans.select().where(volunteer_plans.c.id == id))

""" 更新活动状态 """
async def update_status(id: str, status: str):
    try:
        await database.execute(volunteer_plans.update().where(volunteer_plans.c.id == id).values({
            'status': status,
            'updated_at': datetime.now()
        }))
        return True
