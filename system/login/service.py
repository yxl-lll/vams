...
from system.user.service import username, add, one
...
async def login(_login: LoginBody):
    try:
        ...
        if verify_password(rsa_decrypt_password(_login.password), user.password):
            return Result.success(data=generate_jwt_token({
                "user_id": user.id,
...
async def register(_register: RegisterBody):
    try:
        ...
        if user is None:
            flag = await add(UserBody(nick_name=_register.nickname,gender="1",
                                      username=_register.username,
                                      password=_register.password,
                                      status='1', role_ids="b2168c4f-b972-4d05-ad1b-ff8558c04388"))
            if flag == True:
