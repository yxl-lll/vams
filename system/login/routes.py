...
from .service import login, register, myinfo, login_plaintext
...
async def to_login(request: Request):
    return templates.TemplateResponse(request=request, name="front/login.html", context={
        "sysname": global_config["sysname"]
    })
...
async def html_render(request: Request, menu_id: str, user_id: Annotated[str, Depends(auth)]):
...
async def upload(file: UploadFile):
    ...
    return Result.success(data={
        "url": file_location.replace(project_root, "").replace("\\", "/"), # 返回相对路径，并统一斜杠方向
...
async def uploadUrl(file: UploadFile):
    ...
    with open(file_location, "wb") as f:
        f.write(await file.read())
    return Result.success(data=file_location.replace(project_root, "").replace("\\", "/"))
