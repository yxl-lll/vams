...
from .model import Menu, MenuBody, MenuQuery
...
async def pages(page: int = 1, limit: int = 10, query_menu: MenuQuery = None):
    total_query, query = FixQuery.query(
        ...
        val=query_menu,
        instance=menu,
        total_query=select(func.count(menu.c.id)).select_from(menu),
        query=select(menu),
    )
...
async def get_menu(user_id: str):
    query = (
        ...
        .join_from(menuRole, userRole, menuRole.c.role_id == userRole.c.role_id)
        .where(userRole.c.user_id == user_id)
        .order_by(menu.c.sort.desc())
    )
