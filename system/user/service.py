...
from .model import UserQuery, UserBody, UpPwdBody
from utils.fix_query import FixQuery
import uuid
from utils.passwd import verify_password, rsa_decrypt_password, get_password_hash
from system.user_role.service import add as add_mr, delete as del_by_user_id
...
async def pages(page: int = 1, limit: int = 10, query_user: UserQuery = None):
    total_query, query = FixQuery.query(UserQuery,val=query_user,instance=user,total_query=select(func.count(user.c.id)).select_from(user),query=select(user))
...
async def upd_password(id: str, _user: UpPwdBody):
    ...
    if verify_password(values['old_password'], user_val.password):
        ...
        await database.execute(f""" update user set password = '{password}' where id = '{id}'""")
