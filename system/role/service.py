...
from .model import RoleQuery, RoleBody
from utils.fix_query import FixQuery
import uuid

from system.menu_role.service import add as add_mr, delete as del_by_role_id, menuRole
from system.menu_role.model import MenuRoleBody

from system.menu.service import lists as list_menu, menu
...
async def pages(page: int = 1, limit: int = 10, query_role: RoleQuery = None):
    total_query, query = FixQuery.query(RoleQuery,val=query_role,instance=role,total_query=select(func.count(role.c.id)).select_from(role),query=select(role))
...
async def get_auth_menu(role_id: str):
    if role_id is not None and role_id != "":
        query = select(menu.c.id).join_from(
...
async def get_auth_role(user_id: str):
    if user_id is not None and user_id != "":
        return await lists(RoleQuery()), await database.fetch_all(select(userRole.c.role_id).where(userRole.c.user_id == user_id))
